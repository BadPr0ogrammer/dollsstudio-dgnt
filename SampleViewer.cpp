...
void GLTFViewer::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        */
        ...
        if (m_Model && m_Model->Scenes.size() > 1)
        {
        ...
        if (m_Model && !m_Model->Animations.empty())
        {
            ImGui::SetNextItemOpen(true, ImGuiCond_FirstUseEver);
...
#    error PBR_MAX_LIGHTS should not be defined here
    ...
    auto RenderModel = [&](GLTF_PBR_Renderer::RenderInfo::ALPHA_MODE_FLAGS AlphaModes) {
        ...
        if (m_Model && m_RenderParams.AlphaModes != GLTF_PBR_Renderer::RenderInfo::ALPHA_MODE_FLAG_NONE)
        {
            if (m_pResourceMgr)
            {
...
void GLTFViewer::Update(double CurrTime, double ElapsedTime)
{
    ...
    if (m_Model && !m_Model->Animations.empty() && m_PlayAnimation)
    {
